...
define("DETECTIONS_PHP_URL", "http://www.familiecoenen.nl/detections/detections3.php");
...
define("CTRL_DIR_SC", "/show_camera");
...
function getGlobRule() {
    $glob_rule = DETECTIONS_EXTENSION;
    return $glob_rule;
}
    ...
    if (is_dir($dir)) {
        ...
        if ($timeslots = glob("*", GLOB_ONLYDIR)) {
            ...
            for($index = 0; $index < $n_timeslots; $index++) {
                ...
                print "<a class='w3-center w3-btn w3-border w3-border-purple w3-margin-bottom w3-margin-right w3-round-xxlarge' href='" . DETECTIONS_PHP_URL . "?timeslot=$dir/$timeslot'>$timeslot</a>";
            }
            print "</div>";
        } else {
    ...
    if (is_dir($timeslot)) {
        ...
        if ($pictures = glob(getGlobRule())) {
            ...
            for($index = 0; $index < $n_pictures; $index++) {
                ...
                print "<a class='w3-center w3-btn w3-border w3-border-purple w3-margin-bottom w3-margin-right w3-round-xxlarge' href='" . DETECTIONS_PHP_URL . "?picture=$timeslot/$picture'>$caption<br>";
        ...
        if (is_dir($picture_base)) {
            ...
            if ($pictures = glob(getGlobRule())) {
                ...
                if ($prev_index >= 0) {
                    ...
                    print "<a class='w3-margin-left w3-margin-right w3-left w3-btn w3-border w3-border-purple w3-round-large' href='" . DETECTIONS_PHP_URL . "?picture=$picture_base/$prev_file'>LATER</a>";
                }
                if ($next_index < $n_pictures) {
                    $next_file = $pictures[$next_index];
                    print "<a class='w3-margin-left w3-margin-right w3-right w3-btn w3-border w3-border-purple w3-round-large' href='" . DETECTIONS_PHP_URL . "?picture=$picture_base/$next_file'>EARLIER</a>";
                }
                print "</div>";
            } else {
                print "<div class='w3-container w3-card w3-center w3-pale-purple w3-text-red w3-margin-top w3-margin-bottom w3-padding-4'>";
...
function presentCtrlDirs($basepath) {
    ...
    if (is_dir($basepath . CTRL_DIR_UI)) {
        print "<a class='$ctrl_dir_style_present' href='" . DETECTIONS_PHP_URL . "?tcd=ui'>user interrupt</a>";
    } else {
        print "<a class='$ctrl_dir_style_not_present' href='" . DETECTIONS_PHP_URL . "?tcd=ui'>user interrupt</a>";
    }
    if (is_dir($basepath . CTRL_DIR_EO)) {
        print "<a class='$ctrl_dir_style_present' href='" . DETECTIONS_PHP_URL . "?tcd=eo'>error occurred</a>";
    } else {
        print "<a class='$ctrl_dir_style_not_present' href='" . DETECTIONS_PHP_URL . "?tcd=eo'>error occurred</a>";
    }
    if (is_dir($basepath . CTRL_DIR_FR)) {
        print "<a class='$ctrl_dir_style_present' href='" . DETECTIONS_PHP_URL . "?tcd=fr'>force reboot</a>";
    } else {
        print "<a class='$ctrl_dir_style_not_present' href='" . DETECTIONS_PHP_URL . "?tcd=fr'>force reboot</a>";
    }
    print "</div>";
}
...
function askPassword() {
    ...
    print "<label for='mypwd'>Password: </label>";
    print "<input type='text' id='mypwd' name='mypwd' value=''>";
    print "<input type='submit' value='show camera footage'>";
    print "</form>";
    print "</div>";
}

function handleCameraFootage() {
    if (isset($_POST["mypwd"])) {
        //if (!extension_loaded('sqlite')) {
        if (!extension_loaded('ssh2')) {
            print "<div class='w3-container w3-card w3-center w3-pale-purple w3-text-purple w3-margin-top w3-margin-bottom w3-padding-4'>";
            //print "Extension sqlite is NOT loaded ";
            print "Extension ssh2 is NOT loaded ";
            print "</div>";
        } else {
            print "<div class='w3-container w3-card w3-center w3-pale-purple w3-text-purple w3-margin-top w3-margin-bottom w3-padding-4'>";
            //print "Extension sqlite is LOADED ";
            print "Extension ssh2 is LOADED ";
            print "</div>";
        }

        /*
        $connection = ssh2_connect('94.208.63.232', 22);
        ssh2_auth_password($connection, 'nano', $_POST['mypwd']);
        $stream = ssh2_exec($connection, 'touch /home/nano/github/home-security/build/show_camera_footage');
        if($stream !== false) {
            print "<div class='w3-container w3-card w3-center w3-pale-purple w3-text-purple w3-margin-top w3-margin-bottom w3-padding-4'>";
            print "ssh command successful";
            print "</div>";
        } else {
            print "<div class='w3-container w3-card w3-center w3-pale-purple w3-text-purple w3-margin-top w3-margin-bottom w3-padding-4'>";
...
if (isset($_GET['picture'])) {
    ...
    print "<a class='w3-margin-bottom w3-margin-right w3-center w3-btn w3-border w3-border-purple w3-round-large' href='" . DETECTIONS_PHP_URL . "'>Time Slots</a>";
    print "<a class='w3-margin-bottom w3-margin-right w3-center w3-btn w3-border w3-border-purple w3-round-large' href='" . DETECTIONS_PHP_URL . "?timeslot=$refesh_page'>Step back</a>";
} else if (isset($_GET['timeslot'])) {
    $refesh_page = $_GET["timeslot"];
    print "<a class='w3-margin-bottom w3-margin-right w3-center w3-btn w3-border w3-border-purple w3-round-large' href='" . DETECTIONS_PHP_URL . "'>Time Slots</a>";
    print "<a class='w3-margin-bottom w3-margin-right w3-center w3-btn w3-border w3-border-purple w3-round-large' href='" . DETECTIONS_PHP_URL . "?timeslot=$refesh_page'>Refresh</a>";
    ...
    print "<a class='w3-margin-bottom w3-margin-right w3-center w3-btn w3-border w3-border-purple w3-round-large' href='" . MAIN_URL . "'>www.familiecoenen.nl</a>";
    print "<a class='w3-margin-bottom w3-margin-right w3-center w3-btn w3-border w3-border-purple w3-round-large' href='" . DETECTIONS_PHP_URL . "'>Refresh</a>";
    print "<a class='w3-margin-bottom w3-margin-right w3-center w3-btn w3-border w3-border-purple w3-round-large' href='" . DETECTIONS_PHP_URL . "?rtspme=me'>Live Camera</a>";
}
print "</header>";
...
if (isset($_GET['picture'])) {
    ...
    presentTimeslots(DETECTIONS_DIRECTORY);
